...
public class Page {
	...
	private String frontPageStr;		//前段页面的底部翻页导航
	...
	public String getFrontPageStr() {
		...
		if(totalResult>0){
			...
			if(currentPage==1){
				sb.append("	<li><a>共<font color=red>"+totalResult+"</font>条</a></li>\n");
				sb.append("	<li><input type=\"number\" value=\"\" id=\"toGoPage\" style=\"height:30px;width:60px;text-align:center;float:left\" placeholder=\"页码\"/></li>\n");
				sb.append("	<li style=\"cursor:pointer;\"><a onclick=\"toTZ();\"  class=\"btn btn-mini btn-success\">跳转</a></li>\n");
				sb.append("	<li><a>首页</a></li>\n");
				sb.append("	<li><a>上页</a></li>\n");
			}else{
			...
			sb.append("function nextPage(page){");
			sb.append("	alert(1);\n");
			sb.append("	if(true && $('#urlInp')){\n");
			sb.append("		var url = $('#urlInp').val();\n");
			sb.append("	alert(url);\n");
			sb.append("		if(url.indexOf('?')>-1){url += \"&"+(entityOrField?"currentPage":"page.currentPage")+"=\";}\n");
			sb.append("		else{url += \"?"+(entityOrField?"currentPage":"page.currentPage")+"=\";}\n");
			sb.append("		url = url + page + \"&" +(entityOrField?"showCount":"page.showCount")+"="+showCount+"\";\n");
		...
		frontPageStr = sb.toString();
		return frontPageStr;
	}

	/**
	 * @param frontPageStr the frontPageStr to set
	 */
	public void setFrontPageStr(String frontPageStr) {
		this.frontPageStr = frontPageStr;
	}
