...
import com.health.system.util.UuidUtil;
	...
	public Object memberRegister() throws Exception{
		...
		try{
			...
			if(!sessioncode.equals(code)){
				...
				UuidUtil uuidHexGenerator = new UuidUtil();
		        String memberNo = uuidHexGenerator.generate();
		        password = new SimpleHash("SHA-1", memberNo, password).toString();	//密码加密
				...
				if(mlist.size() > 0){
					...
					Integer id = memberService.insertMember(pd);
					pd.put("id", id);
					//shiro管理的session
	...
	public Object addAddress() throws Exception{
		...
		try{
			...
			if(isdefault.equals("1")){//将其他的默认地址去掉
				PageData pd1 = deliveryAddressService.findDefaultAddress();
				if(pd1 != null){
					pd1.put("isDefault", "2");
